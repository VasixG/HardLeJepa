...
def train_pure_sigreg(cfg: TrainConfig) -> None:
    set_seed(cfg.seed)
    ...
    sigreg = SIGReg(cfg.emb_dim, num_slices=cfg.sigreg_num_slices, t_min=cfg.sigreg_t_min, t_max=cfg.sigreg_t_max, t_steps=cfg.sigreg_t_steps).to(device)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if epoch % cfg.eval_every_epochs == 0:
            ...
            if epoch % cfg.report_every_epochs == 0:
                save_pca_scatter(z_test, y_test, path=str(report_dir / f"epoch{epoch:03d}_pca.png"), title=f"pure_sigreg | epoch {epoch} | PCA (test)", max_points=cfg.report_max_points)
                save_confusion_matrix(knn_cm, path=str(report_dir / f"epoch{epoch:03d}_knn_cm.png"), title=f"pure_sigreg | epoch {epoch} | kNN confusion", normalize=True)
                if epoch % cfg.retrieval_every_epochs == 0:
                    save_retrieval_collage(encoder, train_plain_loader, test_plain_loader, device=device, path=str(report_dir / f"epoch{epoch:03d}_retrieval.png"), title=f"pure_sigreg | epoch {epoch} | retrieval (test->train)", bank_max=cfg.retrieval_bank_max, query_num=cfg.retrieval_query_num, topk=cfg.retrieval_topk)
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(
        ...
        logdir=(args.logdir or f"runs/pure_sigreg_seed{args.seed}"),
        ckpt_path=(args.ckpt or f"checkpoints/pure_sigreg_seed{args.seed}.pt"),
    )
    train_pure_sigreg(cfg)
