    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if epoch % cfg.eval_every_epochs == 0:
            ...
            if epoch % cfg.report_every_epochs == 0:
                save_pca_scatter(z_test, y_test, path=str(report_dir / f"epoch{epoch:03d}_pca.png"), title=f"hardmaxqr+ | epoch {epoch} | PCA (test)", max_points=cfg.report_max_points)
                save_confusion_matrix(knn_cm, path=str(report_dir / f"epoch{epoch:03d}_knn_cm.png"), title=f"hardmaxqr+ | epoch {epoch} | kNN confusion", normalize=True)
                if epoch % cfg.retrieval_every_epochs == 0:
                    save_retrieval_collage(encoder, train_plain_loader, test_plain_loader, device=device, path=str(report_dir / f"epoch{epoch:03d}_retrieval.png"), title=f"hardmaxqr+ | epoch {epoch} | retrieval (test->train)", bank_max=cfg.retrieval_bank_max, query_num=cfg.retrieval_query_num, topk=cfg.retrieval_topk)
...
def parse_args():
    ...
    p.add_argument("--rand", type=int, default=16)
    return p.parse_args()

if __name__ == "__main__":
    args = parse_args()
    cfg = TrainConfig(
        method="train_pure_sigreg_RandomHardMaxQR",
        seed=args.seed,
        device=args.device,
        epochs=args.epochs,
        ...
        logdir=(args.logdir or f"runs/train_pure_sigreg_RandomHardMaxQR_seed{args.seed}"),
        ckpt_path=(args.ckpt or f"checkpoints/train_pure_sigreg_RandomHardMaxQR_seed{args.seed}.pt"),
    )
    train(cfg)
